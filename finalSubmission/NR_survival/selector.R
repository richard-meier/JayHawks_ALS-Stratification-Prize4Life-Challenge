...
train_not_imputed = read.csv("/home/rama.raghavan/als_submission/NatRegSurvival/singularDataTable_reg.csv")
...
#(1)	Surv(time = time_event, event = status) ~ priorSlope + onsetAge + ALS_Staging_Total + priorSlope*ALS_Staging_Total + Age + respiratory
#(2)	Surv(time = time_event, event = status) ~ priorSlope + onsetAge + Q1_Speech + Gender 
#(3)	Surv(time = time_event, event = status) ~ priorSlope + onsetAge + hands + MEDHx_Thyroid 
...
if(predictedGroupLabel == 1){
    feature_name = c("ALS_Staging_Total","respiratory","ALSFRS_Total","Age","onset_delta")
} else if(predictedGroupLabel == 2){
    feature_name = c("Q1_Speech","Gender","ALSFRS_Total","Age","onset_delta")
} else if(predictedGroupLabel == 3){
    feature_name = c("hands","MEDHx_Thyroid","ALSFRS_Total","Age","onset_delta")
}
