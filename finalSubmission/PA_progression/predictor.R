...
load("/home/rama.raghavan/als_submission/ProActProgression/clusterModels_APA.RData")

timePoints = c(12, 18, 24) * 30.5
if(cluster == 111){
	result = predict(modelForCluster_111, newdata = patient_imputed, se.fit=TRUE)
	avgSlp = avgSlp111
} else if(cluster == 112){
    result = predict(modelForCluster_112, newdata = patient_imputed, se.fit=TRUE)
	avgSlp = avgSlp112
} else if(cluster == 12){
    result = predict(modelForCluster_12, newdata = patient_imputed, se.fit=TRUE)
	avgSlp = avgSlp12
} else if(cluster == 21){
    result = predict(modelForCluster_21, newdata = patient_imputed, se.fit=TRUE)
	avgSlp = avgSlp21
} else if(cluster == 22){
    result = predict(modelForCluster_22, newdata = patient_imputed, se.fit=TRUE)
	avgSlp = avgSlp22
} else if(cluster == 3){
    result = predict(modelForCluster_3, newdata = patient_imputed, se.fit=TRUE)
	avgSlp = avgSlp3
} else if(cluster == 4){
    result = predict(modelForCluster_4, newdata = patient_imputed, se.fit=TRUE)
	avgSlp = avgSlp4
}
