...
for(i in features_with_less_than_25perc_missing){
    ...
    #snames = names(table(featTable$SubjectID))
    #patSet = union(patSet, snames)
...
cat(patientSet, patient_imputed$SubjectID[1], "\n")
...
if(predictedGroupLabel == 1){
    feature_name = c("ALS_Staging_Com", "ALSFRS_Total", "Q1_Speech", "Q9_Climbing_Stairs", "hands")
} else if(predictedGroupLabel == 2){
    feature_name = c("Q1_Speech", "Gender", "Q4_Handwriting", "MEDHx_Diabetes", "ALS_Staging_Total", "ALS_Staging_Move")
} else if(predictedGroupLabel == 3){
    feature_name = c("onset_site", "ALS_Staging_Eat", "priorSlope", "Q6_Dressing_and_Hygiene", "trunk", "Gender")
}
...
write.table(outdat,file=paste("/home/rama.raghavan/als_submission/NatRegProgression/",basename(input_file_path),sep=""),sep="|",row.names=FALSE,col.names=F,quote=F)
